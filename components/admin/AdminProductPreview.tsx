...
export function AdminProductPreview({
  ...
  return (
    <div
      className={`bg-black min-h-screen transition-all duration-500 mx-auto overflow-hidden border border-white/10 shadow-2xl ${
        viewMode === "mobile" ? "max-w-[375px] rounded-[3rem]" : "max-w-full"
      }`}
    >
      {/* Simulation of the Store Layout */}
      <div
        className={`${viewMode === "mobile" ? "scale-[0.95]" : "scale-100"} origin-top transition-transform h-full overflow-y-auto custom-scrollbar`}
      >
        <div
          className={`py-12 px-6 ${viewMode === "mobile" ? "space-y-8" : "grid grid-cols-2 gap-12 items-start"}`}
        >
          {/* Images */}
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-2">
              {[1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="aspect-[4/5] bg-white/5 border border-white/5 opacity-40"
                />
              ))}
            </div>
          </div>
          ...
          <div className="flex flex-col">
            ...
            <div className="flex items-center gap-2 mb-6">
              <div className="flex gap-0.5">
                {[1, 2, 3, 4, 5].map((s) => (
                  <Star key={s} size={8} className="fill-accent text-accent" />
                ))}
              </div>
            ...
            <div className="mb-8">
              ...
              <div className="flex gap-2">
                {(product.opcoesTamanho || ["S", "M", "L"]).map((s) => (
                  <div
                    key={s}
                    className="w-8 h-8 rounded-md border border-white/10 flex items-center justify-center text-[10px] text-white/60"
                  >
                    {s}
                  </div>
                ))}
            ...
            <div className="space-y-3 mb-8">
              <div className="w-full py-3 bg-white text-black text-[10px] font-black uppercase text-center rounded-md">
                Add to Cart
              </div>
              <div className="w-full py-3 border border-white/20 text-white text-[10px] font-black uppercase text-center rounded-md">
                Checkout Now
              </div>
            </div>
            ...
            <div className="mt-8 pt-8 border-t border-white/5">
              <h4 className="text-[8px] font-black uppercase text-white/30 mb-6 tracking-widest">
                Heritage FAQ
              </h4>
              <div className="space-y-4">
